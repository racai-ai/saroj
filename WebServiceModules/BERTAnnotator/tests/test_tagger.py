...
def test_person():
    input_text = 'Totodată, petenta a arătat că intimata a emis procesul verbal fără să facă un control fizic, ' + \
        ...
        'de a face obiecțiuni la conținutul prezentului proces verbal, șoferul Vasile Abrudan nu este reprezentantul ' + \
    ...
    assert input_text[result[63][0]:result[64][1]] == 'Vasile Abrudan'
    assert result[77][2] == 'B-PER' and result[78][2] == 'I-PER'
    assert input_text[result[77][0]:result[78][1]] == 'Florin Georgescu'


def test_organization():
    input_text = 'Pe rol judecarea cauzei civile privind pe petent BARIOBAND SRL, pe ' + \
    ...
    assert input_text[result[8][0]:result[11][1]] == 'BARIOBAND SRL'
...
def test_location():
    ...
    assert result[15][2] == 'B-LOC' and result[31][2] == 'I-LOC'
    assert input_text[result[15][0]:result[31][1]
                      ] == 'Belciug, strada Brusturelui nr. 13, com. Ghiminați, jud. Bihor'
